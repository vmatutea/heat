...
//							},
			...
			dataObj.graph2 = new Dygraph(document.getElementById("div_g19"),
					dataObj.sum_lbr, {
						colors	   : new Array( "rgb(0,169,212)","rgb(137,186,23)", "rgb(0,98,95)"),
						drawPoints : true,
					//	fillGraph: true,
...
function updatingData() {
	var timeIndex = state.timeIndexL;
	...
	var active = window.intervalId = setInterval(function() {
		...
		if (isIntervalworking) {
			...
			if (state.timeIndexL != undefined) {
				q1 += " WHERE `timestamp`>" + state.timeIndexL;
				
				state.timeIndexL = t;
			}
			q1 += " ORDER BY `timestamp` ASC";
			
			if (state.timeIndexR != undefined) {
				q2 += " WHERE `timestamp`>" + state.timeIndexR;
				var d = new Date();
				//state.timeIndexR = d.getTime();
			}
			...
			$.ajax({
				...
				if (l != 0) {
					...
					for ( var i = 0; i < l; i++) {
						...
						//state.timeIndexL++;
						console.log("updatingData calling setFolloingData...data");
						setFollowingData(data[i][1]);
					} // for
					console.log("updatingData calling setFolloingData...-2");
					setFollowingData(-2);					

				} else{
					console.log("updatingData calling setFolloingData...-1");
					setFollowingData(-1);
			...
			//		checkLBRs();
					...
					for(var k = 0; k<ues.length; k++){
						if(ues[k].lbr.length != 0){
							ues[k].lbr.push([ ues[k].lbr[ues[k].lbr.length-1][0]+ state.interval, null, null]);
						}
					}
					if(dataObj.sum_lbr.length !=0){
						dataObj.sum_lbr.push([dataObj.sum_lbr[dataObj.sum_lbr.length-1][0]+state.interval/1000,null, null,null]);
					}
				}// if
					limitPresentData();
//
...
function setNewData(obj, data){
	...
	if( obj.buffering.length == 0 || obj.buffering[obj.buffering.length-1][3]!=null || nullers){
		if(st == 1 ){
			console.log("setNewData:",val/1,"start UC");
			obj.buffering.push([d,val/1, null,null]);
		}else if( st == 2){
			console.log("setNewData:",val/1,"start BC");
	...
	if (data[5] == 2){
		console.log("setNewData: The player has stopped");
		if(data[2]==2){
			obj.bc_end = d;
		}
	...
	//switch uc/bc, both graph gets value, don't case wether 0 or 1
	 if( lst ==1 && st ==2){
		 obj.bc_start = d;
		 obj.bc_end = undefined;
		 console.log("setNewData:",val/1, val/1,"Switch from UC to BC"); 
		 obj.buffering.push([d, val/1, val/1, null]);
		 
	 }else if( lst ==2 && st ==1 ){
		 obj.bc_end = d;
		 console.log("setNewData:",val/1, val/1,"Switch from BC to UC");
		 obj.buffering.push([d, val/1, val/1, null]);
		 ...
		 if(lst==0){
			 ...
			 obj.freeze_timestamp = d;
		       	console.log("setNewData:",1,"BC.Buffer underrun");
			 obj.buffering.push([d,null,1,null]);
		 }else if( data[5] == 1){
			 obj.freeze_count++;
			 var ti = d - obj.freeze_timestamp;
			 obj.freeze_timestamp += ti;
			console.log("setNewData:",0,"BC.Playing");
			 obj.buffering.push([d,null,0,null] );
		 }
			console.log("setNewData:",val/1,"BC");
			 obj.buffering.push([d, null, val/1, null]);
	 }
	obj.status = data[2];
	...
	for( var k = 0; k <ues.length; k++){
		if(k != u && k != -2){
			...
			if(ues[k].buffering.length !=0 ){
				...
				var nd =[];
				 if( 0 < u && u<7 ){
					 nd[0] = state.timeIndexL;
					 
					// ues[k].buffering.push()
				 }else{
					 nd[0] = old[0]+ state.interval;
				 }
				...
				ues[k].buffering.push([state.timeIndexL, null, null, null]);
			}
		}
		limitPresentData();
		if (k=0){
		console.log("setFollowingData:",ues[k].buffering);
   		}
