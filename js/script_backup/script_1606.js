...
function setNewData(obj, data){
	...
	var nullers = false;
	if(obj.buffering.length !=0){
		var lastData = obj.buffering[obj.buffering.length-1];
		if(lastData[1] ==null && lastData[2] == null && lastData[3]==null){
			nullers = true;
		}
	}
	
	if( obj.buffering.length == 0 || obj.buffering[obj.buffering.length-1][3]!=null || nullers){
		if(st == 1 ){
...
function setCellOccupancy(data){
	...
	if(data[1] == 1 || data[1]==2 || data[1] == 3 || data[1] == -1){	
		if(data[2]==2){
			for(var j = 0; j<3; j++){
				var ob = ues[j];
				if(ob.bc_start == undefined || data[0] < ob.bc_start || (ob.bc_end != undefined && ob.bc_end < data[0])){
						isBC = true;
