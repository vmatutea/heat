...
// ########################### Link Bit Rate ##################################################################### 
			...
			if ( ConsolBC == true ) console.log("updatingData,beforeAJAX q2",q2);
 			...
 			.fail(function(){
                        console.log("updatingData,AJAX CALL server_data FAIL at timeIndexR+timeIndexRDelta:",timeIndexR+timeIndexRDelta);
                        })
			.done(function(data) {
				if (data[0] == "<") {
					var data = $.parseJSON($(data).html());
		...
		for ( var k=0 ; k < ues.length ; k++ ) {
			...
			if ( last_status_bc != -1 ) { // BC playing
				ues[k].lbr.push([timeIndexR,-1/1,data[3]/1]);
				 ...
				 if ( last_status_uc/1 == -1 ) {
                                ues[k].lbr.push([timeIndexR,-1/1,-1/1]);
                                if ( ConsolBC == true ) console.log("SetNewLBR,BC no-playing,push:timeIndexR,-1/1,-1/1",k,timeIndexR,last_status_uc/1,-1/1);
				}
			}  //else
		} // for
	} else { // UC LBR
...
function setCellOccupancy(data){
	...
	// Timespamp exist? if not push and initilize with UC -1 and BC -1
	if (  CellOccupancy.lbr123[CellOccupancy.lbr123.length-1][0] != timeInt ) {
		CellOccupancy.lbr123.push([timeInt,-1/1,-1/1]);
	        if ( ConsolCO == true ) console.log("CO,old timestamp, push :new timestamp,-1,-1",CellOccupancy.lbr123[CellOccupancy.lbr123.length-2][0],timeInt);
	} //if	
	 if (  CellOccupancy.lbr456[CellOccupancy.lbr456.length-1][0] != timeInt ) {
                CellOccupancy.lbr456.push([timeInt,-1/1,-1/1]);
	}  //if CO.lbr456 furhter populated with data to avoid buffer underrun-> not further considered in this function
	...
	// graphCO_BC_ues123
		if ( ue1.buffering[ue1.buffering.length-1][2] != -1 || ue2.buffering[ue2.buffering.length-1][2] != -1 || ue3.buffering[ue3.buffering.length-1][2] != -1 ) { // status BC playout
			if ( CellOccupancy.lbr123[CellOccupancy.lbr123.length-1][2] == -1 ) {
			CellOccupancy.lbr123[CellOccupancy.lbr123.length-1][2] = data[3]/1;
			if ( ConsolCO == true ) console.log("CO, BC 123 :timeInt, data[3]/1+1",timeInt,data[3]/1+1);
			} else { /// rear case should not happen.... 
			//CellOccupancy.lbr123[CellOccupancy.lbr123.length-1][2] += data[3]/1;
                        CellOccupancy.lbr123[CellOccupancy.lbr123.length-1][2] = data[3]/1;
                        if ( ConsolCO == true ) console.log("CO, BC 123 :timeInt, data[3]/1",timeInt,data[3]/1);
			}	
		}	
	} else { // UC
	// graphCO_UC_ues123
		if ( data[1] == 1 || data[1] == 2 || data[1] == 3 ) { // UE1,2,3 
			if ( ues[data[1]-1].buffering[ues[data[1]-1].buffering.length-1][1] != -1 ) { // check UEx status -> UC playout
